...
from .chat import Citation, get_chat_service, reset_chat_service
...
app = FastAPI(
    title="LangChain + Qwen RAG API",
    version="0.1.0",
    description=(
        "基于 LangChain、DashScope (Qwen) 与 ChromaDB 的检索增强问答服务。"
...
def delete_document(file_hash: str, delete_file: bool = Query(True)) -> dict:
    ...
    - 始终清理 Chroma + nodes.json 中的所有匹配节点
...
async def upload_documents(
    ...
    # 入库新数据后让下次请求重新加载 retriever（BM25 节点列表已变）
    reset_doc_store()
    reset_retriever()
    reset_chat_service()
