...
class Settings(BaseSettings):
    ...
    rerank_model: str = Field("gte-rerank-v2", alias="RERANK_MODEL")
    ...
    def docstore_dir(self) -> Path:
        """BM25 复用所需的"节点快照"目录，单独放一份 nodes.json。"""
        return self.storage_dir / "docstore"

    @property
    def docstore_file(self) -> Path:
        """节点快照 JSON 路径，BM25 重启时直接从此文件复活。"""
        return self.docstore_dir / "nodes.json"
