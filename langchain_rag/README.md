...
一个基于 **LangChain + 阿里云百炼 (Qwen)** 的检索增强问答 (RAG) 项目。
开箱即用地集成了：
...
- 🔍 **混合检索**：向量检索 + BM25（中文 jieba 分词）+ Reciprocal Rank Fusion
- 🎯 **DashScope Rerank**：`gte-rerank-v2` 精排
- 💬 **多轮对话**：自定义"浓缩历史 + 检索上下文"流程，自动改写独立检索问句
...
> ⚠️ 推荐运行环境：**Python 3.11**（LangChain / chromadb / dashscope 在 3.11 上最稳）。
...
成功后会在 `storage/chroma/` 与 `storage/docstore/nodes.json` 看到持久化文件。
...
- **已上传文件列表**：每行显示
  - 🟢/⚪ 入库状态、文件大小、入库时间
  - 📦 在向量库中占多少节点 + 占比
  - 🗑️ **联动删除**：同时删除原始文件 + Chroma 中对应节点 + nodes.json 中对应节点
...
- **召回详情面板**：每条回答下可展开查看 vector / BM25 / 融合 / rerank 四阶段的命中分数与片段预览
...
# -> {"status":"ok","version":"0.1.0","llm_model":"qwen-plus",
#     "embedding_provider":"dashscope","vector_count":15}
```
...
# 按 file_hash 删除（同时清 Chroma + nodes.json + 原始文件）
...
## 7. 与 LlamaIndex 版本的对照

本仓库另有一份基于 **LlamaIndex** 的等价实现（在 `../llamaIndex_rag/` 目录），
功能完全一致。两者的核心差异：

| 维度 | LangChain 版（本目录） | LlamaIndex 版 |
| --- | --- | --- |
| 多轮对话 | 自定义"浓缩历史 + 检索上下文"流程 | `CondensePlusContextChatEngine` |
| 向量检索 | 原生 `chromadb.query`（无中间层） | `VectorStoreIndex.as_retriever` |
| 混合融合 | 手写 RRF（不依赖 EnsembleRetriever） | `QueryFusionRetriever(mode="reciprocal_rerank")` |
| BM25 复用 | 自建 `nodes.json` 节点快照 | LlamaIndex `SimpleDocumentStore` |
| Rerank | 直接调 `dashscope.TextReRank.call` | `DashScopeRerank` 后处理器 |
| LLM | `langchain_community.ChatTongyi` | `llama_index.llms.dashscope.DashScope` |
...
点对应文件后面的 🗑️ 按钮即可联动清理 Chroma + nodes.json。命令行用户也可以直接
...
A: 建议清空 `storage/` 和 `data/uploads/` 后重新上传一次，所有功能即可正常使用。

**Q: rerank 返回失败 / status_code != 200？**
...
**Q: 流式输出 / 多轮中途突然返回空？**
A: 这通常是底层 LLM SDK 的偶发抖动。本项目的流式分支检测到"流没抛异常但
零字符"时会自动用非流式 `invoke()` 重试一次，把答案一次性"流"出来。如果你
看到日志里有 `非流式重试成功`，说明兜底生效了。
